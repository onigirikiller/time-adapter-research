...
DATA_OUT = ROOT / "data/omni3b_delayed_backchannel_v1"
OUT_DIR = ROOT / "artifacts/omni3b_delayed_backchannel_tuning_v1"
FIG_DIR = ROOT / "output/figures/omni3b_delayed_backchannel_tuning_v1"
...
def delayed_soft_label(profile: str, seconds: float) -> tuple[str, list[float], list[str]]:
    if profile == "asked_wait":
        ...
        return "WAIT", dist(0.75, 0.25, 0.0), ["WAIT", "BACKCHANNEL"]

    if profile == "self_repair":
        if seconds < 1.0:
            return "WAIT", dist(0.9, 0.1, 0.0), ["WAIT"]
        return "BACKCHANNEL", dist(0.2, 0.75, 0.05), ["WAIT", "BACKCHANNEL"]
    ...
    if profile == "vulnerable":
        ...
        if seconds < 1.0:
            return "WAIT", dist(0.62, 0.35, 0.03), ["WAIT", "BACKCHANNEL"]
        if seconds < 2.5:
            return "BACKCHANNEL", dist(0.10, 0.70, 0.20), ["BACKCHANNEL", "SUPPORT"]
        return "SUPPORT", dist(0.02, 0.18, 0.80), ["BACKCHANNEL", "SUPPORT"]

    if profile == "neutral_incomplete":
        if seconds < 1.0:
            return "WAIT", dist(0.85, 0.14, 0.01), ["WAIT"]
        if seconds < 4.0:
            return "BACKCHANNEL", dist(0.18, 0.72, 0.10), ["WAIT", "BACKCHANNEL"]
        return "SUPPORT", dist(0.05, 0.35, 0.60), ["BACKCHANNEL", "SUPPORT"]

    if profile == "hesitant":
        if seconds < 1.0:
            return "WAIT", dist(0.86, 0.13, 0.01), ["WAIT", "BACKCHANNEL"]
        if seconds < 3.0:
            return "BACKCHANNEL", dist(0.10, 0.76, 0.14), ["BACKCHANNEL"]
        return "SUPPORT", dist(0.03, 0.30, 0.67), ["BACKCHANNEL", "SUPPORT"]

    if profile == "summary":
        if seconds < 1.0:
            return "WAIT", dist(0.78, 0.20, 0.02), ["WAIT", "BACKCHANNEL"]
        if seconds < 6.0:
            return "BACKCHANNEL", dist(0.12, 0.72, 0.16), ["BACKCHANNEL", "SUPPORT"]
        return "SUPPORT", dist(0.03, 0.30, 0.67), ["BACKCHANNEL", "SUPPORT"]
...
def relabel_rows(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        ...
        new["labeling_policy"] = "delayed_backchannel_v1_no_runtime_gate"
        out.append(new)
    return out
...
def main():
    ...
    manifest = {
        ...
        "labeling_policy": "delayed_backchannel_v1",
        ...
        "note": "Runtime timing gates are not encoded here; delayed behavior is learned via hard and soft target labels.",
    }
    ...
    curves_path = FIG_DIR / "delayed_training_curves.png"
    transitions_path = FIG_DIR / "delayed_test_transitions.png"
    prob_path = FIG_DIR / "delayed_profile_probabilities.png"
    ...
    ckpt_path = OUT_DIR / "adapter_proxy_delayed_audio_only_layer-20.pt"
    ...
    summary = {
        ...
        "notes": [
            "No runtime minimum-silence gate is trained into the demo; delayed behavior is represented by labels and soft labels.",
